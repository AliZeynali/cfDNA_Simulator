...
def get_read_in_range(all_reads, window_size, list_of_n, p_tumor, list_of_positions_errors):
    ...
    if list_of_n != None:
        while len(all_reads) > 0:
            ...
            window_read_tumor = []
            window_read_normal = []
            if len(list_of_positions_errors) > 0 and begin_pos <= list_of_positions_errors[0] <= end_pos:
                list_val = list_of_positions_errors.pop(0)
                new_position_error.append([list_val[0], p_normal * list_val[1] + p_tumor * list_val[2]])
...
if __name__ == "__main__":
    ...
    all_reads = {}
    # TODO p_tumor , p_nomral, list_of_n should be obtained by the code
    list_of_p_tumor = read_percentage_list('ctPercentage_sample.csv')
    list_of_normal_error_rates = read_percentage_list('bloodSmallPeak.csv')
    list_of_tissue_error_rates = read_percentage_list('tissueSmallPeak.csv')
    window_size = 300
    p_tumor = 0.2
    ...
    for chr in all_reads:
        ...
        positions = numpy.random.choice(range(begin_pos, end_pos), int(total_positions / 23), replace=False)
        ...
        for i in range(len(positions)):
            error_positions.append([positions[i], list_of_normal_error_rates.pop(0), list_of_tissue_error_rates.pop(0)])
